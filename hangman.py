...
# game loop
# def main():
#     global FPS, WHITE, BLACK, hangman_status, run, word, clock, guessed, RADIUS, WIDTH, HEIGHT, win
while run:
    clock.tick(FPS)
    draw()
    # events
    ...
    if won:
        ...
        break
        win.fill(BLACK)
        text = RESULT_FONT.render('Press q to quit. Click with mouse to restart.', 1, BLACK)
        win.blit(text, (int(WIDTH/2 - int(text.get_width()/2)), int(HEIGHT/2) - int(text.get_height()/2)))
        pygame.display.update()
